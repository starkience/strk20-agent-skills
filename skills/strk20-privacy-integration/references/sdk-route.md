...
## Sub-accounts (Branch D) — not builder-ready
...
**Status**: contract work exists, but wallet support, SDK support, and an additional Privacy Wallet API call are all still required before builder-facing integration. There is nothing to ship yet.

**What a plan does with Branch D**: scope and track. Identify which of the team's flows want unlinkable accounts (e.g. private collateral positions, private staking), note them as sub-account candidates with an entry criterion ("when sub-accounts reach builder availability"), and route everything buildable today through Branches A–C. Do not name specific partner protocols as launch examples.
